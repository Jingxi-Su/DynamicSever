  ...
  if (path === "/sign_in" && method === "POST") {
    ...
    request.on("end", () => {
      ...
      if (user === undefined) {
        ...
        session[random] = {user_id: user.id}//把id存在随机数里面
        fs.writeFileSync('./session.json', JSON.stringify(session))
        response.setHeader("Set-Cookie", `session_id=${random}; HttpOnly`);//不允许前端触碰cookie,随机数防止用户篡改id
      }
      response.end()
    });
    ...
    if (sessionId && session[sessionId]) {
      ...
      const homeHtml = fs.readFileSync("./public/home.html").toString();//为了下面替换信息读的
      let string = ''
      if (user) {
